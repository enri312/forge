...
pub async fn cmd_tree(project_dir: &Path) -> anyhow::Result<()> {
    let config = ForgeConfig::load(project_dir)?;

    println!(
        "{} {}",
        "🌲".green(),
        format!("Árbol de dependencias para '{}'", config.project.name).bold()
    );

    if config.dependencies.is_empty() && config.test_dependencies.is_empty() {
        println!(
            "   {}",
            "No hay dependencias declaradas en este proyecto.".dimmed()
        );
        return Ok(());
    }
    ...
    println!(
        "   {}",
        "      se visualiza resolviéndolas durante 'forge build'.".dimmed()
    );
